...
const Budget = () => {
    ...
    useEffect(() => {
        const fetchBudgets = async () => {
            try {
                const resBudgets = await axios.get(
                    'http://127.0.0.1:8000/budgets/?skip=0&limit=10&sort_by=CreatedAt&sort_order=desc',
                    { headers }
                );
                ...
                const budgetsWithDetails = await Promise.all(
                    budgetList.map(async (budget) => {
                        const overviewRes = await axios.get(
                            `http://127.0.0.1:8000/budgets/${budget.BudgetID}/overview`,
                            { headers }
                        );

                        let vsActualData = [];
                        try {
                            const vsActualRes = await axios.get(
                                `http://127.0.0.1:8000/budgets/${budget.BudgetID}/vs-actual`,
                                { headers }
                            );
